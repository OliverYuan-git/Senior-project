...
def generate_cluster_data(n=1000, d=2, random_seed=None):
    if random_seed is not None:
        np.random.seed(random_seed)
    negative_samples = np.random.rand(4 * n // 5, d)
    #postive
    s = 0.1 
    positive_cluster_1 = np.random.rand(n // 10, d) * s
    positive_cluster_2 = np.random.rand(n // 10, d)
...
X, y = generate_cluster_data(n=1000, d=2, random_seed=42)
plt.scatter(X[y == 1][:, 0], X[y == 1][:, 1], c='red', label='Positive', marker='x')
plt.scatter(X[y == 0][:, 0], X[y == 0][:, 1], c='blue', label='Negative', marker='o')
...
def wide_reach_classification(X, y, theta=0.99, epsilon_p=0.01, epsilon_n=0.01, epsilon_r=0.01):
