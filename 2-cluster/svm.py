...
# print SVM
plt.figure(figsize=(8, 6))
plt.scatter(X_test[y_test == 0][:, 0], X_test[y_test == 0][:, 1], c='blue', label='Negative Samples')
plt.scatter(X_test[y_test == 1][:, 0], X_test[y_test == 1][:, 1], c='red', label='Positive Samples')
plt.scatter(X_test[y_pred != y_test][:, 0], X_test[y_pred != y_test][:, 1], facecolors='none', edgecolors='k', s=100, label='Misclassified')
plt.legend()
plt.title('SVM Classification Results with RBF Kernel on 2-Cluster Data')
plt.xlabel('Dimension 1')
plt.ylabel('Dimension 2')
plt.grid(True)
plt.show()
